...
def compare_branches(branch1: str, branch2: str):
    ...
    # Also get direct diff between branches
    result = subprocess.run(
    ...
    for filepath in sorted(all_files):
        ...
        if content1 == content2:
            ...
            lines1 = [normalize_line(l) for l in content1.split('\n')]
            lines2 = [normalize_line(l) for l in content2.split('\n')]
    ...
    if binary_files:
        print(f"\nBinary files (skipped): {len(binary_files)}")

    print("\n" + "=" * 60)
    total_files = len(identical) + len(minor_diffs) + len(significant_diffs)
    print(f"Summary: {total_files} files compared")
    if len(significant_diffs) == 0 and len(only_in_1) == 0 and len(only_in_2) == 0:
