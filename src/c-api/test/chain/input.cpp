...
static kth_output_point_mut_t make_outpoint(void) {
    kth_output_point_mut_t op = kth_domain_chain_output_point_construct_from_hash_index(&kPrevHash, 7);
    REQUIRE(op != NULL);
    return op;
}
...
static kth_input_mut_t make_default_input(void) {
    ...
    kth_input_mut_t in = kth_domain_chain_input_construct(op, script, 0u);
    REQUIRE(in != NULL);
...
TEST_CASE("C-API Input - field constructor preserves fields",
    ...
    kth_output_point_mut_t op = make_outpoint();
    kth_script_mut_t script = make_script();
    kth_input_mut_t in = kth_domain_chain_input_construct(op, script, kSequence);
    REQUIRE(kth_domain_chain_input_sequence(in) == kSequence);
    kth_domain_chain_input_destruct(in);
    kth_domain_chain_script_destruct(script);
    kth_domain_chain_output_point_destruct(op);
}
...
TEST_CASE("C-API Input - destruct null is safe", "[C-API Input]") {
    kth_domain_chain_input_destruct(NULL);
}
...
TEST_CASE("C-API Input - from_data insufficient bytes fails",
    ...
    kth_error_code_t ec = kth_domain_chain_input_construct_from_data(
...
TEST_CASE("C-API Input - to_data / from_data roundtrip", "[C-API Input]") {
    kth_output_point_mut_t op = make_outpoint();
    kth_script_mut_t script = make_script();
    kth_input_mut_t expected = kth_domain_chain_input_construct(op, script, kSequence);

    kth_size_t size = 0;
    uint8_t* raw = kth_domain_chain_input_to_data(expected, 1, &size);
    REQUIRE(raw != NULL);
    REQUIRE(size > 0);

    kth_input_mut_t parsed = NULL;
    kth_error_code_t ec = kth_domain_chain_input_construct_from_data(raw, size, 1, &parsed);
    REQUIRE(ec == kth_ec_success);
    REQUIRE(parsed != NULL);
    REQUIRE(kth_domain_chain_input_sequence(parsed) == kSequence);
    REQUIRE(kth_domain_chain_input_equals(expected, parsed) != 0);

    kth_core_destruct_array(raw);
    kth_domain_chain_input_destruct(parsed);
    kth_domain_chain_input_destruct(expected);
    kth_domain_chain_script_destruct(script);
    kth_domain_chain_output_point_destruct(op);
}
...
TEST_CASE("C-API Input - sequence setter roundtrip", "[C-API Input]") {
    kth_input_mut_t in = make_default_input();
    REQUIRE(kth_domain_chain_input_sequence(in) != kSequence);
    kth_domain_chain_input_set_sequence(in, kSequence);
    REQUIRE(kth_domain_chain_input_sequence(in) == kSequence);
    kth_domain_chain_input_destruct(in);
}
...
TEST_CASE("C-API Input - script setter roundtrip", "[C-API Input]") {
    kth_input_mut_t in = make_default_input();
    kth_script_mut_t script = make_script();
    kth_domain_chain_input_set_script(in, script);
    ...
    kth_script_const_t view = kth_domain_chain_input_script(in);
    REQUIRE(view != NULL);
    REQUIRE(kth_domain_chain_script_equals(view, script) != 0);
    kth_domain_chain_input_destruct(in);
    kth_domain_chain_script_destruct(script);
}
...
TEST_CASE("C-API Input - previous_output setter roundtrip", "[C-API Input]") {
    kth_input_mut_t in = make_default_input();
    kth_output_point_mut_t op = make_outpoint();
    kth_domain_chain_input_set_previous_output(in, op);
    // Read the borrowed view back and assert it equals what we wrote.
    // The default outpoint has a zero hash and index 0, so a no-op
    // setter would not match the kPrevHash / index 7 fixture.
    kth_output_point_const_t view = kth_domain_chain_input_previous_output(in);
    REQUIRE(view != NULL);
    REQUIRE(kth_domain_chain_output_point_equals(view, op) != 0);
    kth_domain_chain_input_destruct(in);
    kth_domain_chain_output_point_destruct(op);
}
...
TEST_CASE("C-API Input - is_final on max sequence is true", "[C-API Input]") {
    kth_input_mut_t in = make_default_input();
    kth_domain_chain_input_set_sequence(in, 0xffffffffu);
    REQUIRE(kth_domain_chain_input_is_final(in) != 0);
    kth_domain_chain_input_destruct(in);
}
...
TEST_CASE("C-API Input - is_final on non-max sequence is false",
          "[C-API Input]") {
    kth_input_mut_t in = make_default_input();
    kth_domain_chain_input_set_sequence(in, 0u);
    REQUIRE(kth_domain_chain_input_is_final(in) == 0);
    kth_domain_chain_input_destruct(in);
}
...
TEST_CASE("C-API Input - signature_operations on default is zero",
          "[C-API Input]") {
    kth_input_mut_t in = make_default_input();
    REQUIRE(kth_domain_chain_input_signature_operations(in, 0, 0) == 0);
    kth_domain_chain_input_destruct(in);
}
...
TEST_CASE("C-API Input - copy preserves fields", "[C-API Input]") {
    kth_output_point_mut_t op = make_outpoint();
    kth_script_mut_t script = make_script();
    kth_input_mut_t original = kth_domain_chain_input_construct(op, script, kSequence);
    kth_input_mut_t copy = kth_domain_chain_input_copy(original);

    REQUIRE(kth_domain_chain_input_equals(original, copy) != 0);
    REQUIRE(kth_domain_chain_input_sequence(copy) == kSequence);

    kth_domain_chain_input_destruct(copy);
    kth_domain_chain_input_destruct(original);
    kth_domain_chain_script_destruct(script);
    kth_domain_chain_output_point_destruct(op);
}
...
TEST_CASE("C-API Input - equals different inputs", "[C-API Input]") {
    kth_output_point_mut_t op = make_outpoint();
    kth_script_mut_t script = make_script();
    kth_input_mut_t a = kth_domain_chain_input_construct(op, script, kSequence);
    kth_input_mut_t b = kth_domain_chain_input_construct(op, script, kSequence);
    kth_input_mut_t c = make_default_input();

    REQUIRE(kth_domain_chain_input_equals(a, b) != 0);
    REQUIRE(kth_domain_chain_input_equals(a, c) == 0);

    kth_domain_chain_input_destruct(a);
    kth_domain_chain_input_destruct(b);
    kth_domain_chain_input_destruct(c);
    kth_domain_chain_script_destruct(script);
    kth_domain_chain_output_point_destruct(op);
}
...
TEST_CASE("C-API Input - construct_from_data null data with non-zero size aborts",
    ...
    KTH_EXPECT_ABORT({
        kth_input_mut_t in = NULL;
        kth_domain_chain_input_construct_from_data(NULL, 1, 1, &in);
    });
}
...
TEST_CASE("C-API Input - construct_from_data null out aborts",
    ...
    uint8_t data[10];
    memset(data, 0, sizeof(data));
    KTH_EXPECT_ABORT(kth_domain_chain_input_construct_from_data(data, 10, 1, NULL));
}

TEST_CASE("C-API Input - to_data null out_size aborts",
          "[C-API Input][precondition]") {
    kth_input_mut_t in = make_default_input();
    KTH_EXPECT_ABORT(kth_domain_chain_input_to_data(in, 1, NULL));
    kth_domain_chain_input_destruct(in);
}

TEST_CASE("C-API Input - copy null self aborts",
          "[C-API Input][precondition]") {
    KTH_EXPECT_ABORT(kth_domain_chain_input_copy(NULL));
}

TEST_CASE("C-API Input - construct null previous_output aborts",
          "[C-API Input][precondition]") {
    kth_script_mut_t script = make_script();
    KTH_EXPECT_ABORT(kth_domain_chain_input_construct(NULL, script, kSequence));
    kth_domain_chain_script_destruct(script);
}
...
TEST_CASE("C-API Input - construct null script aborts",
          "[C-API Input][precondition]") {
    kth_output_point_mut_t op = make_outpoint();
    KTH_EXPECT_ABORT(kth_domain_chain_input_construct(op, NULL, kSequence));
    kth_domain_chain_output_point_destruct(op);
}
