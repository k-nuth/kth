...
TEST_CASE("C-API InputList - default construct is empty",
    ...
    REQUIRE(list != NULL);
    REQUIRE(kth_domain_chain_input_list_count(list) == 0u);
    kth_domain_chain_input_list_destruct(list);
}

TEST_CASE("C-API InputList - destruct null is safe",
          "[C-API InputList]") {
    kth_domain_chain_input_list_destruct(NULL);
}
...
TEST_CASE("C-API InputList - push_back / count / nth",
    ...
    REQUIRE(kth_domain_chain_input_list_count(list) == 1u);

    kth_input_const_t elem = kth_domain_chain_input_list_nth(list, 0);
    REQUIRE(elem != NULL);
...
TEST_CASE("C-API InputList - erase removes element",
    ...
    kth_domain_chain_input_list_push_back(list, in);
    kth_domain_chain_input_list_push_back(list, in);
    REQUIRE(kth_domain_chain_input_list_count(list) == 2u);
    kth_domain_chain_input_list_erase(list, 0);
    REQUIRE(kth_domain_chain_input_list_count(list) == 1u);
    kth_domain_chain_input_destruct(in);
    kth_domain_chain_input_list_destruct(list);
}
...
TEST_CASE("C-API InputList - nth out of bounds aborts",
    ...
    kth_input_list_mut_t list = kth_domain_chain_input_list_construct_default();
    KTH_EXPECT_ABORT(kth_domain_chain_input_list_nth(list, 0));
    kth_domain_chain_input_list_destruct(list);
}
