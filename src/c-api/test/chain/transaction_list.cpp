...
TEST_CASE("C-API TransactionList - default construct is empty",
    ...
    kth_transaction_list_mut_t list = kth_domain_chain_transaction_list_construct_default();
    REQUIRE(list != NULL);
    REQUIRE(kth_domain_chain_transaction_list_count(list) == 0u);
    kth_domain_chain_transaction_list_destruct(list);
}

TEST_CASE("C-API TransactionList - destruct null is safe",
          "[C-API TransactionList]") {
    kth_domain_chain_transaction_list_destruct(NULL);
}
...
TEST_CASE("C-API TransactionList - push_back / count / nth",
    ...
    kth_domain_chain_transaction_list_push_back(list, tx);
    REQUIRE(kth_domain_chain_transaction_list_count(list) == 1u);

    kth_domain_chain_transaction_list_push_back(list, tx);
    REQUIRE(kth_domain_chain_transaction_list_count(list) == 2u);

    kth_transaction_const_t elem = kth_domain_chain_transaction_list_nth(list, 0);
    REQUIRE(elem != NULL);
...
TEST_CASE("C-API TransactionList - erase removes element",
    ...
    kth_domain_chain_transaction_list_push_back(list, tx);
    REQUIRE(kth_domain_chain_transaction_list_count(list) == 2u);
    kth_domain_chain_transaction_list_erase(list, 0);
    REQUIRE(kth_domain_chain_transaction_list_count(list) == 1u);
    kth_domain_chain_transaction_destruct(tx);
    kth_domain_chain_transaction_list_destruct(list);
}
...
TEST_CASE("C-API TransactionList - nth out of bounds aborts",
    ...
    kth_transaction_list_mut_t list = kth_domain_chain_transaction_list_construct_default();
    KTH_EXPECT_ABORT(kth_domain_chain_transaction_list_nth(list, 0));
    kth_domain_chain_transaction_list_destruct(list);
}
