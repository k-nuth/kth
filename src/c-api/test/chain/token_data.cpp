...
TEST_CASE("C-API TokenData - destruct null is safe", "[C-API TokenData]") {
    kth_domain_chain_token_data_destruct(NULL);
}
...
TEST_CASE("C-API TokenData - make_fungible builds a valid handle",
          "[C-API TokenData]") {
    kth_token_data_mut_t td = kth_domain_chain_token_make_fungible(&kCategoryA, 1234u);
    REQUIRE(td != NULL);
    REQUIRE(kth_domain_chain_token_data_is_valid(td) != 0);
    REQUIRE(kth_domain_chain_token_data_get_kind(td) == kth_token_kind_fungible_only);
    REQUIRE(kth_domain_chain_token_data_is_fungible_only(td) != 0);
    REQUIRE(kth_domain_chain_token_data_has_nft(td) == 0);
    REQUIRE(kth_domain_chain_token_data_get_amount(td) == 1234);
    REQUIRE(kth_hash_equal(kth_domain_chain_token_data_id(td), kCategoryA) != 0);
...
TEST_CASE("C-API TokenData - make_fungible_unsafe builds the same handle",
          "[C-API TokenData]") {
    kth_token_data_mut_t safe = kth_domain_chain_token_make_fungible(&kCategoryA, 7u);
    kth_token_data_mut_t unsafe = kth_domain_chain_token_make_fungible_unsafe(kCategoryA.hash, 7u);

    REQUIRE(safe != NULL);
    REQUIRE(unsafe != NULL);
    REQUIRE(kth_domain_chain_token_data_equals(safe, unsafe) != 0);

    kth_domain_chain_token_data_destruct(unsafe);
    kth_domain_chain_token_data_destruct(safe);
}
...
TEST_CASE("C-API TokenData - make_non_fungible mutable carries commitment and capability",
          "[C-API TokenData]") {
    kth_token_data_mut_t td = kth_domain_chain_token_make_non_fungible(
        &kCategoryA, kth_token_capability_mut, kCommitment, sizeof(kCommitment));
    REQUIRE(td != NULL);

    REQUIRE(kth_domain_chain_token_data_is_valid(td) != 0);
    REQUIRE(kth_domain_chain_token_data_get_kind(td) == kth_token_kind_non_fungible_only);
    REQUIRE(kth_domain_chain_token_data_has_nft(td) != 0);
    REQUIRE(kth_domain_chain_token_data_is_fungible_only(td) == 0);
    REQUIRE(kth_domain_chain_token_data_is_mutable_nft(td) != 0);
    REQUIRE(kth_domain_chain_token_data_is_immutable_nft(td) == 0);
    REQUIRE(kth_domain_chain_token_data_is_minting_nft(td) == 0);

    REQUIRE(kth_domain_chain_token_data_get_nft_capability(td) == kth_token_capability_mut);

    kth_size_t commitment_size = 0;
    uint8_t* commitment = kth_domain_chain_token_data_get_nft_commitment(td, &commitment_size);
...
TEST_CASE("C-API TokenData - make_both carries amount, capability, commitment",
          "[C-API TokenData]") {
    kth_token_data_mut_t td = kth_domain_chain_token_make_both(
        &kCategoryA, 9999u, kth_token_capability_mut, kCommitment, sizeof(kCommitment));
    REQUIRE(td != NULL);

    REQUIRE(kth_domain_chain_token_data_is_valid(td) != 0);
    REQUIRE(kth_domain_chain_token_data_get_kind(td) == kth_token_kind_both);
    REQUIRE(kth_domain_chain_token_data_has_nft(td) != 0);
    REQUIRE(kth_domain_chain_token_data_is_fungible_only(td) == 0);
    REQUIRE(kth_domain_chain_token_data_is_mutable_nft(td) != 0);

    REQUIRE(kth_domain_chain_token_data_get_amount(td) == 9999);
    REQUIRE(kth_domain_chain_token_data_get_nft_capability(td) == kth_token_capability_mut);
...
TEST_CASE("C-API TokenData - make_fungible rejects amount 0",
    ...
    kth_token_data_mut_t td = kth_domain_chain_token_make_fungible(&kCategoryA, 0u);
    REQUIRE(td == NULL);
}
...
TEST_CASE("C-API TokenData - make_fungible rejects amounts above INT64_MAX",
    ...
    kth_token_data_mut_t td = kth_domain_chain_token_make_fungible(
...
TEST_CASE("C-API TokenData - get_amount returns 0 for pure NFT",
    ...
    kth_token_data_mut_t td = kth_domain_chain_token_make_non_fungible(
        &kCategoryA, kth_token_capability_none, kCommitment, sizeof(kCommitment));
    REQUIRE(td != NULL);
    REQUIRE(kth_domain_chain_token_data_get_amount(td) == 0);
    kth_domain_chain_token_data_destruct(td);
}

TEST_CASE("C-API TokenData - get_nft_capability is `none` for pure fungible",
          "[C-API TokenData]") {
    kth_token_data_mut_t td = kth_domain_chain_token_make_fungible(&kCategoryA, 1u);
    REQUIRE(td != NULL);
    REQUIRE(kth_domain_chain_token_data_get_nft_capability(td) == kth_token_capability_none);
    kth_domain_chain_token_data_destruct(td);
}
...
TEST_CASE("C-API TokenData - copy preserves equality", "[C-API TokenData]") {
    kth_token_data_mut_t original = kth_domain_chain_token_make_both(
        &kCategoryA, 42u, kth_token_capability_mut, kCommitment, sizeof(kCommitment));
    REQUIRE(original != NULL);

    kth_token_data_mut_t copy = kth_domain_chain_token_data_copy(original);
    REQUIRE(copy != NULL);
    REQUIRE(kth_domain_chain_token_data_equals(original, copy) != 0);

    kth_domain_chain_token_data_destruct(copy);
    kth_domain_chain_token_data_destruct(original);
}

TEST_CASE("C-API TokenData - different categories compare unequal",
          "[C-API TokenData]") {
    kth_token_data_mut_t a = kth_domain_chain_token_make_fungible(&kCategoryA, 1u);
    kth_token_data_mut_t b = kth_domain_chain_token_make_fungible(&kCategoryB, 1u);

    REQUIRE(kth_domain_chain_token_data_equals(a, b) == 0);

    kth_domain_chain_token_data_destruct(b);
    kth_domain_chain_token_data_destruct(a);
}

TEST_CASE("C-API TokenData - different amounts compare unequal",
          "[C-API TokenData]") {
    kth_token_data_mut_t a = kth_domain_chain_token_make_fungible(&kCategoryA, 1u);
    kth_token_data_mut_t b = kth_domain_chain_token_make_fungible(&kCategoryA, 2u);
...
TEST_CASE("C-API TokenData - to_data / from_data roundtrip (fungible)",
          "[C-API TokenData]") {
    kth_token_data_mut_t original = kth_domain_chain_token_make_fungible(&kCategoryA, 7777u);
    REQUIRE(original != NULL);

    kth_size_t size = 0;
    uint8_t* raw = kth_domain_chain_token_data_to_data(original, &size);
    REQUIRE(raw != NULL);
    REQUIRE(size == kth_domain_chain_token_data_serialized_size(original));

    kth_token_data_mut_t parsed = NULL;
    kth_error_code_t ec = kth_domain_chain_token_construct_from_data(raw, size, &parsed);
    REQUIRE(ec == kth_ec_success);
    REQUIRE(parsed != NULL);
    REQUIRE(kth_domain_chain_token_data_equals(original, parsed) != 0);

    kth_domain_chain_token_data_destruct(parsed);
...
TEST_CASE("C-API TokenData - from_data with truncated input fails",
    ...
    kth_token_data_mut_t parsed = NULL;
    kth_error_code_t ec =
        kth_domain_chain_token_construct_from_data(truncated, sizeof(truncated), &parsed);
    REQUIRE(ec != kth_ec_success);
    REQUIRE(parsed == NULL);
}
...
TEST_CASE("C-API TokenData - set_id replaces category", "[C-API TokenData]") {
    ...
    kth_domain_chain_token_data_set_id(td, &kCategoryB);
    REQUIRE(kth_hash_equal(kth_domain_chain_token_data_id(td), kCategoryB) != 0);
...
TEST_CASE("C-API TokenData - copy null aborts",
    ...
    KTH_EXPECT_ABORT(kth_domain_chain_token_data_copy(NULL));
}

TEST_CASE("C-API TokenData - get_kind null aborts",
          "[C-API TokenData][precondition]") {
    KTH_EXPECT_ABORT(kth_domain_chain_token_data_get_kind(NULL));
}

TEST_CASE("C-API TokenData - construct_from_data null out aborts",
          "[C-API TokenData][precondition]") {
    KTH_EXPECT_ABORT(
        kth_domain_chain_token_construct_from_data(NULL, 0, NULL));
}

TEST_CASE("C-API TokenData - construct_from_data non-null *out aborts",
          "[C-API TokenData][precondition]") {
    kth_token_data_mut_t already = kth_domain_chain_token_make_fungible(&kCategoryA, 1u);
    KTH_EXPECT_ABORT(
        kth_domain_chain_token_construct_from_data(NULL, 0, &already));
    kth_domain_chain_token_data_destruct(already);
}

TEST_CASE("C-API TokenData - to_data null out_size aborts",
          "[C-API TokenData][precondition]") {
    kth_token_data_mut_t td = kth_domain_chain_token_make_fungible(&kCategoryA, 1u);
    KTH_EXPECT_ABORT(kth_domain_chain_token_data_to_data(td, NULL));
    kth_domain_chain_token_data_destruct(td);
}

TEST_CASE("C-API TokenData - make_fungible_unsafe null id aborts",
          "[C-API TokenData][precondition]") {
    KTH_EXPECT_ABORT(kth_domain_chain_token_make_fungible_unsafe(NULL, 1u));
}

TEST_CASE("C-API TokenData - make_fungible null id aborts",
          "[C-API TokenData][precondition]") {
    KTH_EXPECT_ABORT(kth_domain_chain_token_make_fungible(NULL, 1u));
}

TEST_CASE("C-API TokenData - make_non_fungible null id aborts",
          "[C-API TokenData][precondition]") {
    KTH_EXPECT_ABORT(kth_domain_chain_token_make_non_fungible(
        NULL, kth_token_capability_none, kCommitment, sizeof(kCommitment)));
}

TEST_CASE("C-API TokenData - make_both null id aborts",
          "[C-API TokenData][precondition]") {
    KTH_EXPECT_ABORT(kth_domain_chain_token_make_both(
...
TEST_CASE("C-API TokenData - set_id null value aborts",
    ...
    kth_token_data_mut_t td = kth_domain_chain_token_make_fungible(&kCategoryA, 1u);
    REQUIRE(td != NULL);
    KTH_EXPECT_ABORT(kth_domain_chain_token_data_set_id(td, NULL));
    kth_domain_chain_token_data_destruct(td);
}
