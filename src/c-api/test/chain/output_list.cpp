...
TEST_CASE("C-API OutputList - default construct is empty",
    ...
    REQUIRE(list != NULL);
    REQUIRE(kth_domain_chain_output_list_count(list) == 0u);
    kth_domain_chain_output_list_destruct(list);
}

TEST_CASE("C-API OutputList - destruct null is safe",
          "[C-API OutputList]") {
    kth_domain_chain_output_list_destruct(NULL);
}
...
TEST_CASE("C-API OutputList - push_back / count / nth",
    ...
    REQUIRE(kth_domain_chain_output_list_count(list) == 1u);

    kth_output_const_t elem = kth_domain_chain_output_list_nth(list, 0);
    REQUIRE(elem != NULL);
    REQUIRE(kth_domain_chain_output_is_valid(elem) != 0);
...
TEST_CASE("C-API OutputList - erase removes element",
    ...
    kth_domain_chain_output_list_push_back(list, out);
    kth_domain_chain_output_list_push_back(list, out);
    REQUIRE(kth_domain_chain_output_list_count(list) == 2u);
    kth_domain_chain_output_list_erase(list, 0);
    REQUIRE(kth_domain_chain_output_list_count(list) == 1u);
    kth_domain_chain_output_destruct(out);
    kth_domain_chain_output_list_destruct(list);
}
...
TEST_CASE("C-API OutputList - nth out of bounds aborts",
    ...
    kth_output_list_mut_t list = kth_domain_chain_output_list_construct_default();
    KTH_EXPECT_ABORT(kth_domain_chain_output_list_nth(list, 0));
    kth_domain_chain_output_list_destruct(list);
}
