...
TEST_CASE("C-API StealthCompact - destruct null is safe", "[C-API StealthCompact]") {
    kth_domain_chain_stealth_compact_destruct(NULL);
}
...
TEST_CASE("C-API StealthCompact - set then get round-trips all fields", "[C-API StealthCompact]") {
    ...
    kth_stealth_compact_list_mut_t list = kth_domain_chain_stealth_compact_list_construct_default();
    REQUIRE(list != NULL);
    ...
    kth_domain_chain_stealth_compact_list_destruct(list);
}
...
TEST_CASE("C-API StealthCompact - copy preserves equality", "[C-API StealthCompact]") {
    ...
    kth_stealth_compact_list_mut_t list = kth_domain_chain_stealth_compact_list_construct_default();
    REQUIRE(list != NULL);
    REQUIRE(kth_domain_chain_stealth_compact_list_count(list) == 0);
    kth_domain_chain_stealth_compact_list_destruct(list);
}
...
TEST_CASE("C-API StealthCompact - copy null aborts",
    ...
    KTH_EXPECT_ABORT(kth_domain_chain_stealth_compact_copy(NULL));
}

TEST_CASE("C-API StealthCompact - getter null aborts",
          "[C-API StealthCompact][precondition]") {
    KTH_EXPECT_ABORT(kth_domain_chain_stealth_compact_ephemeral_public_key_hash(NULL));
}
...
TEST_CASE("C-API StealthCompactList - destruct null is safe", "[C-API StealthCompactList]") {
    kth_domain_chain_stealth_compact_list_destruct(NULL);
}
