...
// Owned: `block` (`kth_block_mut_t`) — caller must release with `kth_domain_chain_block_destruct`.
typedef void (*kth_block_fetch_handler_t)(kth_chain_t, void*, kth_error_code_t, kth_block_mut_t block, kth_size_t);

// Owned: `header` (`kth_header_mut_t`) — caller must release with `kth_domain_chain_header_destruct`.
...
// Owned: `block` (`kth_compact_block_mut_t`) — caller must release with `kth_domain_message_compact_block_destruct`.
...
// Owned: `spend` (`kth_input_point_mut_t`) — caller must release with `kth_domain_chain_input_point_destruct`.
typedef void (*kth_spend_fetch_handler_t)(kth_chain_t, void*, kth_error_code_t, kth_input_point_mut_t spend);

// Owned: `tx` (`kth_transaction_mut_t`) — caller must release with `kth_domain_chain_transaction_destruct`.
...
// Owned: `incoming` (`kth_block_list_mut_t`) — caller must release with `kth_domain_chain_block_list_destruct`.
// Owned: `replaced` (`kth_block_list_mut_t`) — caller must release with `kth_domain_chain_block_list_destruct`.
...
// Owned: `ds_proof` (`kth_double_spend_proof_mut_t`) — caller must release with `kth_domain_message_double_spend_proof_destruct`.
