...
public:
    ...
    /// Store an address (returns true if stored, false if duplicate/invalid).
    virtual
    bool store(address const& address);

    /// Store a collection of addresses (returns count stored).
    virtual
    size_t store(address::list const& addresses);
    ...
    virtual
    code fetch_addresses(address::list& out_addresses, size_t count) const;

    /// Remove an address (returns true if removed).
    virtual
    bool remove(address const& address);
