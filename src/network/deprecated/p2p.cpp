...
void p2p::handle_manual_started(code const& ec, result_handler handler) {
    ...
    // hosts_ loads from file in constructor, no explicit start needed
...
bool p2p::stop() {
    // hosts_ saves to file in destructor, no explicit stop needed
...
bool p2p::store(address const& address) {
    return hosts_.store(address);
}

size_t p2p::store(address::list const& addresses) {
    return hosts_.store(addresses);
}
...
code p2p::fetch_addresses(address::list& out_addresses, size_t count) const {
    return hosts_.fetch(out_addresses, count);
}

bool p2p::remove(address const& address) {
