 ...
 * This includes additional fields for flags and expected error codes.
 */
struct bchn_script_test {
    ...
    uint64_t amount{0};             ///< Input amount in satoshis (for SIGHASH_FORKID tests)
};
...
script_test_list const invalid_context_free_scripts{
    ...
    // 0xc1 (active_bytecode) is a valid introspection opcode with bch_native_introspection
    {"1", "if 0xc2 else 1 endif", "opcode 0xc2 invalid if executed"},
    // 0xc3 (tx_input_count) is a valid introspection opcode with bch_native_introspection
    ...
    // 64-bit arithmetic overflow/underflow (invalid before and after GAUSS activation, valid with BigInt)
    // {"9223372036854775807 1", "add nop", "64-bit overflow: 2^63-1 + 1 = 2^63 exceeds range — valid with bch_bigint"},
};
...
// Auto-generated BCHN script tests are now in script.cpp (individual TEST_CASEs)
