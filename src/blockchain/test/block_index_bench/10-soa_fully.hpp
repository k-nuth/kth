...
#ifndef KTH_BLOCKCHAIN_TEST_BLOCK_INDEX_BENCH_10_SOA_FULLY_HPP
#define KTH_BLOCKCHAIN_TEST_BLOCK_INDEX_BENCH_10_SOA_FULLY_HPP
...
// =============================================================================
// Implementation: SoA Fully - Every field in its own vector
// Maximum cache efficiency for traversal operations
//
// Each field is stored in a separate vector:
...
//
// Benefits:
//   - Traversal only touches parent_indices_ (4-byte stride)
//   - GetMedianTimePast only touches parent_indices_ + timestamps_
...
class block_index_store {
    ...
    // === Structure of Arrays: each field in its own vector ===

    // Traversal data (hot path)
    std::vector<uint32_t> parent_indices_;   // 4 bytes per block
    std::vector<uint32_t> skip_indices_;     // 4 bytes per block - for O(log n) ancestor lookup

    // Block metadata
    std::vector<int32_t> heights_;           // 4 bytes per block
    std::vector<uint64_t> chain_works_;      // 8 bytes per block
    std::vector<uint32_t> statuses_;         // 4 bytes per block

    // Header fields (without prev_block_hash - redundant with parent_indices_)
    std::vector<uint32_t> versions_;         // 4 bytes per block
    std::vector<hash_digest> merkle_roots_;  // 32 bytes per block
    std::vector<uint32_t> timestamps_;       // 4 bytes per block
    std::vector<uint32_t> bits_;             // 4 bytes per block
    std::vector<uint32_t> nonces_;           // 4 bytes per block

    // Hash lookup
    boost::unordered_flat_map<hash_digest, uint32_t, hash_digest_hasher> hash_to_idx_;

public:
    // Pre-allocate storage for all vectors
    void preallocate(size_t n) {
        std::unique_lock lock(mutex_);
        parent_indices_.reserve(n);
        skip_indices_.reserve(n);
        heights_.reserve(n);
        chain_works_.reserve(n);
        statuses_.reserve(n);
        versions_.reserve(n);
        merkle_roots_.reserve(n);
...
private:
    uint32_t add_internal(hash_digest const& hash, header_data const& hdr) {
        ...
        // Store each field in its own vector (all vectors stay in sync)
...
public:
    ...
    // Accessors (no lock - caller must ensure thread safety)
    ...
    // Only touches parent_indices_ + timestamps_ vectors
    // =========================================================================
    ...
    // Only touches skip_indices_ + parent_indices_ + heights_ vectors
    // =========================================================================
...
#endif  // KTH_BLOCKCHAIN_TEST_BLOCK_INDEX_BENCH_10_SOA_FULLY_HPP
