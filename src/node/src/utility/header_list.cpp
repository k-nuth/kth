...
hash_digest header_list::previous_hash() const {
    ...
    return list_.empty() ? start_.hash() : kth::domain::chain::hash(list_.back());
    ///////////////////////////////////////////////////////////////////////////
}
...
bool header_list::link(const domain::chain::header& header) const {
    ...
    return header.previous_block_hash() == kth::domain::chain::hash(list_.back());
}
...
bool header_list::check(header const& header) const {
    ...
    auto const hash = kth::domain::chain::hash(header);
    return ! header.check(hash, retarget);
}
...
bool header_list::accept(header const& header) const {
    ...
    return remaining() > 1 || kth::domain::chain::hash(header) == stop_.hash();
}
