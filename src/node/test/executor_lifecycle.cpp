...
// The subprocess harness these cases run on — claimed_dir, watchdog, run_child,
// require_clean_child, announce_completion — now lives in a header of its own,
// because p2p_node's teardown controls need exactly the same thing: a stop that
// does not end a wait leaves join() blocked, and an in-process watchdog that
// hard-exits would take the whole suite with it.
