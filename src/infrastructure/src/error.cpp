...
std::string error_category_impl::message(int ev) const noexcept {
    static const std::unordered_map<int, std::string> messages = {
        ...
        { error::invalid_tx_input_index, "invalid_tx_input_index" },
        { error::invalid_tx_output_index, "invalid_tx_output_index" },
        ...
        // Chip VM limits
        { error::op_cost_limit, "op cost limit exceeded" },
        { error::too_many_hash_iters, "too many hash iters" },
        { error::conditional_stack_depth, "conditional stack depth" },

        // Transaction structure
        { error::invalid_tx_version, "invalid transaction version" },

        // Cash Tokens
        { error::invalid_bitfield, "invalid token bitfield" },
        { error::token_amount_negative, "token amount negative" },
        { error::token_fungible_only_amount_zero, "fungible-only token amount is zero" },
        { error::token_amount_bitfield_mismatch, "token amount bitfield mismatch" },
        { error::token_commitment_bitfield_mismatch, "token commitment bitfield mismatch" },
        { error::token_fungible_with_commitment, "fungible-only token with commitment" },
        { error::token_commitment_oversized, "token commitment oversized" },
        { error::token_coinbase_has_tokens, "coinbase transaction has token outputs" },
        { error::token_unparseable_output, "unparseable token data in output" },
        { error::token_unparseable_input, "unparseable token data in input" },
        { error::token_input_created_pre_activation, "token input created before activation" },
        { error::token_inputs_missing_or_spent, "token inputs missing or spent" },
        { error::token_duplicate_genesis, "token duplicate genesis category" },
        { error::token_invalid_category, "token invalid category" },
        { error::token_fungible_insufficient, "token fungible amount insufficient" },
        { error::token_nft_ex_nihilo, "token NFT created ex nihilo" },
        { error::token_amount_overflow, "token amount overflow" },
        { error::token_pre_activation_input, "token input spent before activation" },

        // Generic script error categories
        { error::insufficient_main_stack, "insufficient main stack" },
        { error::invalid_operand_size, "invalid operand size" },
        { error::insufficient_alt_stack, "insufficient alt stack" },
        { error::unbalanced_conditional, "unbalanced conditional" },
        { error::division_by_zero, "division by zero" },
        { error::verify_failed, "verify failed" },
